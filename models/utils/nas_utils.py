...
class ACBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, dilation=1, groups=1, deploy=False):
        ...
        if deploy:
            ...
            self.ver_conv = nn.Conv2d(in_channels=in_channels, out_channels=out_channels,
                                      ...
                                      stride=stride,
                                      padding=ver_pad_or_crop, dilation=dilation, groups=groups, bias=False)
            self.hor_conv = nn.Conv2d(in_channels=in_channels, out_channels=out_channels,
                                      kernel_size=(1, kernel_size),
                                      stride=stride,
                                      padding=hor_pad_or_crop, dilation=dilation, groups=groups, bias=False)
    ...
    def forward(self, input):
        if hasattr(self, 'fused_conv'):
            ...
            return self.nonlinearity(square_outputs + vertical_outputs + horizontal_outputs + id_out)
    ...
    def switch_to_deploy(self):
        ...
        for para in self.parameters():
            para.detach_()
        for attr in ['square_conv', 'square_bn', 'ver_conv', 'ver_bn', 'hor_conv', 'hor_bn']:
